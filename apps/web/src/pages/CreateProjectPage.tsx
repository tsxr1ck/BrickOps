...
const titleStyle: CSSProperties = { fontSize: 'var(--bo-text-xl)', fontWeight: 'var(--bo-weight-bold)' as any, color: 'var(--bo-text-primary)' };
...
const labelStyle: CSSProperties = { fontSize: 'var(--bo-text-sm)', fontWeight: 'var(--bo-weight-medium)' as any, color: 'var(--bo-text-primary)' };
const inputBase: CSSProperties = { width: '100%', padding: 'var(--bo-space-3) var(--bo-space-4)', borderRadius: 'var(--bo-radius-md)', border: '1px solid var(--bo-border)', background: 'var(--bo-bg-secondary)', color: 'var(--bo-text-primary)', fontSize: 'var(--bo-text-base)', outline: 'none', minHeight: 'var(--bo-tap-target)', boxSizing: 'border-box' };
const selectStyle: CSSProperties = { ...inputBase, appearance: 'none', paddingRight: '36px' };
const toggleGroup: CSSProperties = { display: 'flex', borderRadius: 'var(--bo-radius-md)', border: '1px solid var(--bo-border)', overflow: 'hidden' };
const toggleBtn = (on: boolean): CSSProperties => ({ flex: 1, padding: 'var(--bo-space-3)', fontSize: 'var(--bo-text-sm)', fontWeight: on ? 'var(--bo-weight-semibold)' as any : 'var(--bo-weight-normal)' as any, background: on ? 'var(--bo-accent-bg)' : 'var(--bo-bg-secondary)', color: on ? 'var(--bo-accent)' : 'var(--bo-text-secondary)', border: 'none', cursor: 'pointer', minHeight: 'var(--bo-tap-target)' });
...
export function CreateProjectPage() {
  ...
  return (
    <form onSubmit={handleSubmit} style={pageStyle}>
      ...
      <Button type="submit" variant="primary" size="lg" fullWidth disabled={!goal.trim()} icon={<Rocket size={18} />} id="submit-new-project">
        Create Project
      </Button>
    </form>
  );
